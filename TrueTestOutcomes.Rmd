...
inputPanel(
  ...
  numericInput("avgE", label = "Average true effect size", value = 1, min = -100, max = 100),
...
observeEvent(c(input$alpha, input$beta, input$mde, input$tails, input$base, input$testCt, input$avgE, input$traffVolume, input$cvValue, input$months), {
  a <- 1 - (input$alpha/100)
  b <- input$beta/100 
...
Row {.topRow}
-----------------------------------------------------------------------
What are the likely true outcomes for a testing program?
...
render_gt({
  gt(simVals$summary) %>%
    ...
    fmt_currency(vars(Value), rows=c(8,9,10), decimals = 0) %>%
...
```{r dollarValue}
renderPlot({
  df <- simVals$summary[8:10,1:2]
  
  ggplot(df, aes(x = Metric)) +
    geom_col(
      mapping = aes(y = Value, fill=Metric),
    )
})
