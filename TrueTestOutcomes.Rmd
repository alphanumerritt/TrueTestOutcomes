...
observeEvent(c(input$simulate, input$dispersion, input$alpha, input$beta, input$mde, input$tails, input$base, input$testCt, input$avgE, input$traffVolume, input$cvValue, input$months, input$testCost), {
  ...
  dispersion <- if (input$dispersion == "High") 750 else if (input$dispersion == "Medium") 2000 else 10000   
...
output$exp2 <- renderUI({
  div("Next, we create random samples for our control and test groups. The control conversion rate is ",
      ...
      strong(paste0(simVals$sample," visitors,")),
...
```{r resultsExplained}
renderUI({
  div(id = "highlights", style = "font-size: 14px; max-width: 300px;",
      ...
      "Not bad!"),
      p("Only ",
        paste0(round(simVals$summary[4,2]*100),"%"),
        " of these were false positives."),
      p("However, ",
        paste0(round(sum(simVals$simulation$trueEff > 0)/input$testCt*100),"%"),
