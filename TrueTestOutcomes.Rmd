...
output$exp3 <- renderUI({
  div(
      ...
      p("Only ",
        ...
        "However, a total of",
        strong(paste0(round(sum(simVals$simulation$trueEff > 0)/input$testCt*100),"%")),
        " of tests actually had true effects > 0. ",
        "So we missed out on some potential winners."
      ),
  )
  )
})
...
renderPlot({
  ...
  ggplot(df, aes(x = Metric, fill = pos)) +
    ...
    geom_text(
      mapping = aes(y = Value+(input$testCt*input$testCost*.1), label = paste0("$",format(Value, big.mark=",", digits = 0, scientific = FALSE)))
    ) +
    theme_light() +
