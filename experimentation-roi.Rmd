...
inputPanel(
  ...
  disabled(
    numericInput("tails", label = "(Locked) How many tails?", value = 1, min = 1, max = 2)
    ),
  numericInput("base", label = "What's the base conversion rate? (%)", value = 5, min = 1, max = 99)
  # numericInput("nonf", label = "WIP - Do you want to use a non-inferiority margin? (%) 1-tail only", value = 0, min = 0, max = 99)
...
Real Outcomes <!-- {data-icon="fa-percentage"} -->
===================================== 
...
Row {data-height=380}
-----------------------------------------------------------------------
### 1st: Simulate true effects {.threecol}
...
### 2nd: Simulate test samples {.threecol}
...
### 3rd: Compare test outcomes to truth {.threecol}
```{r resultsExplained}
...
Row {data-height=300}
-----------------------------------------------------------------------
### ROI Component 1: Upside From Winners {.threecol}

- No. of winners
- $ value of winners
- Compare to estimated
- Min/Max effect and avg. effect
- Lift we missed out on too

### ROI Component 2: Savings From Avoided Losers {.threecol}

- No. of losers avoided
- $ value of losers

### ROI Component 3: Behavioral Changes From Having Insurance {.threecol}

- Did we take bigger risks? [this could widen dispersion]
- Did we invest less up front (use MVP)? [this could change cost structure]
- Did we do more stuff?

Row {data-height=150}
----------------------------------------------------------------------

# For something totally different
- Enter total costs of program
- Enter investments (kind of same as total costs), get test outputs
- Output range of outcomes vs single one (probabilities)
- Take info from actual experiments ran
- % ROI
- Make it so people can't hack it to get what they want
- Decay rate
# Or
- Choose time period
- Give each test a random end date
- Apply roi to time period
- Distinguish realized ROI and unrealized ROI
- Assign a decay rate
- Opportunity cost of testing (could do more without it)
